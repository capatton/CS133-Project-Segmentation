...
//contour = empty when passed in
//u = img, already filled out
//m = width,
//n = height
//MaxIter = iterations
int segmentation(float* contour, const float* u, int m, int n, int MaxIter) {
	...
	//init phi array and curv array
	for(i=0; i < m; i++) {
		for(j=0; j < n; j++) {
			float xx = i;
			float yy = j;
			//phi(i, j) = square root of (current i - center of x + current j - center of y) - radius 
			phi(i, j)	= sqrtf(SQR(xx-xcent) + SQR(yy-ycent)) - r;
			curv(i, j)	= 0;
		}
	}
	...
	for(iter=0; iter<MaxIter; iter++) {
		...
		// calculate c1, c2 from num1, den1, num2, den2
		for(i=0; i<m; i++) {
			for(j=0; j < n; j++) {
				if(phi(i,j) < 0) {
					num1 += 256*u(i,j);
	...
	//assign contour values based on the values of phi
	for(i=1; i<m; i++) {
		for (j=1; j<n; j++) {
			if (phi(i, j)*phi(i-1, j)<0 || phi(i, j)*phi(i, j-1)<0) 
				contour[i*n+j] = 0.99;
